...
function moveToOriginalPosition() { // 선택된 top5 아래의 하나의 이미지를 제자리로 이동시킴
	...
	// 1. imgElement의 순위를 나타내는 <span>을 삭제
	$(imgElement).prev(".rank").remove();

    // 2. cds의 <img>들 중 id 값이 imgElement의 id 값보다 크면서 가장 가까운 것을 찾아
    // imgElement를 cds의 제 위치에 삽입
    var id = parseInt($(imgElement).attr("id"));
    var targetImg = $("#cds > img").filter(function() {
        return parseInt($(this).attr("id")) > id;
    }).first();
    if (targetImg.length) {
        $(imgElement).insertBefore(targetImg);
    } else {
        // 3. cds의 <img>들 중 id 값이 imgElement보다 큰 것이 없을 경우
        // 가장 마지막에 삽입
        $("#cds").append(imgElement);
    }

    // 4. top5 아래의 imgElement 다음에 있었던 이미지들의 순위 값을 변경
    var rankSpans = $("#top5 .rank");
	rankSpans.each(function() {
	var rank = parseInt($(this).text());
    if (rank >= id) { // id보다 큰 순위를 가진 이미지들에 대해 순위를 1씩 감소시킴
        $(this).text(rank - 1);
    	}
	});

    // 5. imgElement에 대해 onclick 이벤트 핸들러 재설정
    $(imgElement).off("click").click(addToTop5);
}
